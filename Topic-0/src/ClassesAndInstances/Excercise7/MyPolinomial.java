...
public class MyPolinomial {
  ...
  public MyPolinomial(String fileName) {
    ...
    for (int i = 0; i < this.coeffs.length; i++) this.coeffs[i] = in.nextDouble();
  }
  ...
  public double evaluate(double x) {
    double result = 0.0;
    for(int i = 0; i < this.coeffs.length; i++) result += this.coeffs[i] * Math.pow(x, i);
    return result;
  }
  ...
  public MyPolinomial add(MyPolinomial another) {
    ...
    for(int i = 0; i < this.coeffs.length; i++) newCoeffs[i] = this.coeffs[i];
    for(int i = 0; i < another.coeffs.length; i++) newCoeffs[i] += another.coeffs[i];
